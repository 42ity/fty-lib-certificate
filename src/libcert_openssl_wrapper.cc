/*  =========================================================================
    libcert_openssl_wrapper - Openssl wrapper for crypto
...
ByteField randomVector(size_t nbBytes)
{
    ...
    if (RAND_bytes(&rndVector[0], static_cast<int>(nbBytes)) == 0) {
...
std::string base64Encode(const ByteField& data)
{
    ...
    BIO_write(b64, &data[0], static_cast<int>(data.size()));
    (void)BIO_flush(b64);
...
ByteField base64Decode(const std::string& encodedData, const size_t off, const size_t count)
{
    ...
    bmem = BIO_new_mem_buf(const_cast<char*>(encodedData.c_str() + off), static_cast<int>(count));
    bmem = BIO_push(b64, bmem);

    BIO_set_flags(bmem, BIO_FLAGS_BASE64_NO_NL);
    size_t sizeData = static_cast<size_t>(BIO_read(bmem, base64.data(), static_cast<int>(encodedData.length())));
...
ByteField Aes256cbcEncrypt(const ByteField& data, const ByteField& key, const ByteField& iv)
{
     ...
     */
    EVP_EncryptUpdate(ctx, &cipherData[0], &len, &data[0], static_cast<int>(data.size()));
    cipherDataLen = static_cast<size_t>(len);
...
ByteField Aes256cbcDecrypt(const ByteField& cipherData, const ByteField& key, const ByteField& iv)
{
    ...
    EVP_DecryptUpdate(ctx, plainData.data(), &len, cipherData.data(), static_cast<int>(cipherData.size()));

    plainDataLen = static_cast<size_t>(len);

...
test_item_t* test_available(const char* testname)
{
    test_item_t* item;
...
{
    test_item_t* item;
    printf("Running fty-lib-certificate selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq(item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test(verbose);
#ifdef FTY_LIB_CERTIFICATE_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_lib_certificate_private_selftest(verbose, item->subtest);
#endif // FTY_LIB_CERTIFICATE_BUILD_DRAFT_API
    }
...
static void test_list(void)
{
    ...
    for (item = all_tests; item->testname; item++)
        printf(
            "    %-40s - %s	%s\n", item->testname, item->stable ? "stable" : "draft", item->pub ? "public" : "private");
}
...
int main(int argc, char** argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        if (streq(argv[argn], "--help") || streq(argv[argn], "-h")) {
            puts("fty_lib_certificate_selftest.c [options] ...");
        ...
        if (streq(argv[argn], "--verbose") || streq(argv[argn], "-v"))
            verbose = true;
        else if (streq(argv[argn], "--number") || streq(argv[argn], "-n")) {
            test_number();
            return 0;
        } else if (streq(argv[argn], "--list") || streq(argv[argn], "-l")) {
            test_list();
            return 0;
        } else if (streq(argv[argn], "--test") || streq(argv[argn], "-t")) {
            argn++;
            if (argn >= argc) {
        ...
        } else if (streq(argv[argn], "--continue") || streq(argv[argn], "-c")) {
#ifdef _MSC_VER
    ...
    if (test) {
        printf("Running fty-lib-certificate test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test(verbose);
#ifdef FTY_LIB_CERTIFICATE_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_lib_certificate_private_selftest(verbose, test->subtest);
#endif // FTY_LIB_CERTIFICATE_BUILD_DRAFT_API
    } else
        test_runall(verbose);
